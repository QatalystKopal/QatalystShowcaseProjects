            ...
            {project.sdgs.slice(0, 6).map((sdg) => 
              sdg.image ? (
                <img
                  ...
                  className="shrink-0 h-8 w-auto rounded object-contain"
                  loading="lazy"
                />
              ) : (
                <div
                  key={sdg.number}
                  title={`SDG ${sdg.number}: ${sdg.label}`}
                  className="shrink-0 flex items-center justify-center w-7 h-7 rounded-full font-bold text-[12px] shadow-sm border border-white/50"
                  style={{
                    background: SDG_COLORS[sdg.number] ?? "#6b7280",
                    color: sdgTextColor(sdg.number),
