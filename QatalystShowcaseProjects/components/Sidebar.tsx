...
const navSections: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: Film,              label: "Video Library",               href: "/videos" },
...
export function Sidebar() {
  ...
  const effectiveVideoLibraryHref = currentProjectId
    ? `/projects/${currentProjectId}/videos`
    : "/videos";

  const isActive = (href: string) => {
    if (href === "/") return pathname === "/";
    if (href === "/projects") return pathname.startsWith("/projects") && !pathname.endsWith("/documents") && !pathname.endsWith("/videos");
    if (href === "/documents") return pathname === "/documents" || pathname.endsWith("/documents");
    if (href === "/videos") return pathname === "/videos" || pathname.endsWith("/videos");
  ...
  function NavLink({
    ...
    const resolvedHref =
      ...
      href === "/videos"    ? effectiveVideoLibraryHref :
      href;
    return (
      <Link
        href={resolvedHref}
  ...
  return (
    <aside
      className="relative shrink-0 flex flex-col overflow-y-auto overflow-x-hidden md:h-full z-40"
      style={{ width, background: "#0e0e16", transition: isDragging.current ? "none" : "width 0ms", height: isMobile ? '80px' : '100%' }}
    >
      {/* ── Logo ── */}
      <div
